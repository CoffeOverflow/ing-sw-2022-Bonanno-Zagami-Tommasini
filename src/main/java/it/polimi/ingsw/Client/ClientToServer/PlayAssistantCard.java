...
public class PlayAssistantCard implements ClientToServerMessage{
    ...
    public void handleMessage(GameHandler game, ClientHandler player){
        ...
        try{
            ...
            System.out.println(game.getController().getCurrentCardPlayers().toString());
            if(game.getController().getCurrentCardPlayers().size()==game.getNumberOfPlayers()) {
                game.getController().getCurrentCardPlayers().clear();
                game.sendTo(new YourTurn(),game.getClientByPlayerID(game.getController().getTurnOrder()[0]));
                game.sendAllExcept(new IsTurnOfPlayer(game.getController().getModel().getPlayerByID(game.getController().getTurnOrder()[0]).getNickname()),
                        game.getClientByPlayerID(game.getController().getTurnOrder()[0]));
